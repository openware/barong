...
- `build-master`
    ...
    - configure barong-repository `uri` to change ssh link to your respository.
    - configure barong-repository `branch` to change building branch of your respository.
...
```shell
fly -t ci set-pipeline -p barong -c config/pipelines/review.yml -n
```

## Un-pause the pipeline
```shell
fly -t ci unpause-pipeline -p barong
```
