if ENV.true?('USE_P2P_SESSION_STORAGE')
  ...
  class ActionDispatch::Session::BzRedisStore <  ActionDispatch::Session::RedisStore
    ...
    SECRET = ENV.fetch('P2P_SESSION_SECRET', 'secret')
  ...
  Rails.application.config.session_store :bz_redis_store,
    ...
    key: ENV.fetch('P2P_SESSION_COOKIE'),
    servers: [ENV.fetch('P2P_SESSION_REDIS_URL', 'redis://localhost:6379'), serializer: Oj]
end
