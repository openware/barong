if ENV['JWT_PUBLIC_KEY'].present?
  jwt_public_key = OpenSSL::PKey.read(Base64.urlsafe_decode64(ENV.fetch('JWT_PUBLIC_KEY')))
...
(YAML.load_file('config/management_api_v1.yml') || {}).deep_symbolize_keys.tap do |x|
  ...
  ManagementAPI::V1::JWTAuthenticationMiddleware.security_configuration = x
end
