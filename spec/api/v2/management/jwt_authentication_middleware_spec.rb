...
describe API::V2::Management::JWTAuthenticationMiddleware, type: :request do
    ...
    it 'works in standard conditions' do
      post_json '/api/v2/timestamp', multisig_jwt_management_api_v1({}, :alex)
    ...
    it 'denies access when token is expired' do
      ...
      post_json '/api/v2/timestamp', multisig_jwt_management_api_v1({ exp: 1.minute.ago.to_i }, :alex)
      expect(response).to have_http_status(401)
    ...
    context 'valid issuer' do
      ...
      it 'validates issuer' do
        post_json '/api/v2/timestamp', multisig_jwt_management_api_v1({ iss: 'qux' }, :alex)
        expect(response).to be_successful
      end
    end
    ...
    context 'invalid issuer' do
      ...
      before { config[:jwt].merge!(iss: 'qux') }
      it 'validates issuer' do
        post_json '/api/v2/timestamp', multisig_jwt_management_api_v1({ iss: 'hacker' }, :alex)
        expect(response).to have_http_status(401)
        expect(response.body).to match(/failed to verify jwt/i)
      end
    end
    ...
    context 'valid audience' do
      ...
      it 'validates audience' do
        post_json '/api/v2/timestamp', multisig_jwt_management_api_v1({ aud: 'qux' }, :alex)
        expect(response).to be_successful
      end
    end
    ...
    context 'invalid audience' do
      ...
      before { config[:jwt].merge!(aud: 'qux') }
      it 'validates audience' do
        post_json '/api/v2/timestamp', multisig_jwt_management_api_v1({ aud: 'hacker' }, :alex)
        expect(response).to have_http_status(401)
        expect(response.body).to match(/failed to verify jwt/i)
      end
    end
    ...
    context 'issued at in future' do
      ...
      it 'denies access' do
        post_json '/api/v2/timestamp', multisig_jwt_management_api_v1({ iat: 200.seconds.from_now.to_i }, :alex)
        expect(response).to have_http_status(401)
        expect(response.body).to match(/failed to verify jwt/i)
      end
    end
    ...
    context 'issued at before future' do
      before { config[:jwt][:verify_iat] = true }
      it 'allows access' do
        post_json '/api/v2/timestamp', multisig_jwt_management_api_v1({ iat: 3.seconds.ago.to_i }, :alex)
