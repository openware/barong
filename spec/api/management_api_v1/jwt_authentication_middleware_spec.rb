...
describe ManagementAPI::V1::JWTAuthenticationMiddleware, type: :request do
  ...
  it 'allows only POST, PUT' do
    ...
    delete '/management_api/v1/timestamp'
    expect(response).to have_http_status(405)
  end
  ...
  it 'requires JSON in the request body' do
    post '/management_api/v1/timestamp', params: 'foo=baz&baz=qux'
    expect(response).to have_http_status(400)
